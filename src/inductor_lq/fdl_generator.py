...
def add_shunt_ground_landing(lines: list[str], objects: list[str], obj_index: int, geom: dict[str, object], params: InductorParams) -> int:
    right_spec = geom["right_bridge_spec"]
    p2x, p2y = bridge_outer_point(right_spec, "right")
    angle = right_spec[4]
    theta = math.radians(angle)
    landing_length = max(96.0, 6.0 * params.width)
    landing_width = max(80.0, 6.0 * params.width)
    cx = p2x + math.cos(theta) * landing_length / 2.0
    cy = p2y + math.sin(theta) * landing_length / 2.0
    landing_spec = (cx, cy, landing_length, landing_width, angle)
    for layer, width in (("m5", landing_width), ("v4", max(landing_width - 8.0, params.wv4)), ("m4", landing_width)):
        obj_index = add_rectangle(lines, objects, obj_index, f"L1_ShuntGroundLanding_{layer}", layer, landing_spec, width)
    return obj_index


def write_fdl(output_path: Path, params: InductorParams, sides: int = 12, mode: str = "series_float") -> None:
    if mode not in {"series_float", "shunt_grounded"}:
        raise ValueError(f"Unsupported FDL mode: {mode}")
    ...
    if mode == "shunt_grounded":
        obj_index = add_shunt_ground_landing(lines, objects, obj_index, geom, params)
